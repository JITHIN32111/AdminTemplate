...
const AdminAside = ({ open, setOpen }) => {
  ...
  const Menus = [
    ...
    { title: "All Transactions", path: "/admin/history", icon: <FaHistory /> },
    { title: "Profile", path: "/admin/edit-profile", icon: <AiOutlineProfile /> },
  ...
  return (
    <div className={`relative ${window.innerWidth >= 768 ? "static" : ""}`}>
      <div
        ...
        } bg-slate-100 h-screen p-3 pt-6 shadow-2xl z-50 ${
          window.innerWidth < 768 ? "fixed top-0" : ""
        }`}
      >
        <ImMenu
          className={`absolute cursor-pointer -right-3 top-9 w-7 text-white bg-blue-700 shadow-2xl
        ...
        <div className="flex gap-x-4 items-center">
          <div className="flex justify-center items-center md:hidden gap-x-3">
            <img
              src={img}
              className={`cursor-pointer duration-500 md:ml-4 w-12 h-12`}
            />
            <h1 className="font-bold text-xl text-blue-700">blizerFI</h1>
          </div>
        </div>
        <ul className="pt-6">
          {Menus.map((Menu, index) => (
            <li
              ...
              className={`flex rounded-md p-2 cursor-pointer hover:bg-light-white ${
                location.pathname === Menu.path ? "bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-indigo-500 hover:to-blue-500 text-white" : "text-blue-700"
